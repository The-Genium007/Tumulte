...
describe("useSupportReporter Composable", () => {
  ...
  describe("Return values", () => {
    test("should return sendBugReport and sendSuggestion functions", async () => {
      const { useSupportReporter } =
        await import("~/composables/useSupportReporter");
  ...
  describe("sendBugReport()", () => {
    describe("Validation", () => {
      test("should throw error if title is too short", async () => {
        ...
        await expect(
          sendBugReport("Hi", "Valid description text"),
        ).rejects.toThrow("Le titre doit faire au moins 5 caractères.");
      });
      ...
      test("should throw error if title is empty", async () => {
        ...
        await expect(
          sendBugReport("", "Valid description text"),
        ).rejects.toThrow("Le titre doit faire au moins 5 caractères.");
      });
    ...
    describe("Successful submission", () => {
      test("should send bug report with diagnostics by default", async () => {
        ...
        const result = await sendBugReport(
          "Test bug title",
          "This is a valid bug description",
        );
    ...
    describe("Error handling", () => {
      test("should throw error on API failure with error message", async () => {
        ...
        await expect(
          sendBugReport("Valid title", "Valid description text"),
      ...
      test("should throw generic error on API failure without error message", async () => {
        ...
        await expect(
          sendBugReport("Valid title", "Valid description text"),
        ).rejects.toThrow("Impossible d'envoyer le rapport de bug.");
      });
  ...
  describe("sendSuggestion()", () => {
    describe("Validation", () => {
      test("should throw error if title is too short", async () => {
        ...
        await expect(
          sendSuggestion("Hi", "Valid suggestion description"),
        ).rejects.toThrow("Le titre doit faire au moins 5 caractères.");
      });
    ...
    describe("Successful submission", () => {
      test("should send suggestion successfully", async () => {
        ...
        expect(result.githubDiscussionUrl).toBe(
          "https://github.com/test/discussions/1",
        );
        expect(result.discordSent).toBe(true);
    ...
    describe("Error handling", () => {
      test("should throw error on API failure", async () => {
        ...
        await expect(
          sendSuggestion("Valid title", "Valid description text"),
      ...
      test("should throw generic error on API failure without message", async () => {
        ...
        await expect(
          sendSuggestion("Valid title", "Valid description text"),
        ).rejects.toThrow("Impossible d'envoyer la suggestion.");
      });
    });
  });
  ...
  describe("Context building", () => {
    ...
    test("should include store state when diagnostics enabled", async () => {
      ...
      expect(callBody.frontend.storeState.pollControl.pollStatus).toBe(
        "running",
      );
      expect(callBody.frontend.storeState.pollControl.launchedPollsCount).toBe(
        3,
      );
    });

...
describe("Campaigns Store", () => {
  ...
  describe("inviteStreamer", () => {
    test("should invite streamer and add to members list", async () => {
      ...
      const { campaignsRepository } =
        await import("~/api/repositories/campaigns_repository");
      vi.mocked(campaignsRepository.inviteStreamer).mockResolvedValueOnce(
        mockMember,
      );
      ...
      expect(campaignsRepository.inviteStreamer).toHaveBeenCalledWith(
        "campaign-1",
        { streamerId: "streamer-123" },
      );
    ...
    test("should handle invite error", async () => {
      ...
      await expect(
        store.inviteStreamer("campaign-1", "invalid-streamer"),
  ...
  describe("removeMember", () => {
    test("should remove member from selected campaign", async () => {
      const { campaignsRepository } =
        await import("~/api/repositories/campaigns_repository");
      vi.mocked(campaignsRepository.removeMember).mockResolvedValueOnce(
        undefined,
      );
      ...
      expect(campaignsRepository.removeMember).toHaveBeenCalledWith(
        "campaign-1",
        "member-1",
      );
    ...
    test("should handle remove error", async () => {
      ...
      await expect(
        store.removeMember("campaign-1", "invalid-member"),
  ...
  describe("fetchCampaign", () => {
    ...
    test("should handle fetch campaign error", async () => {
      const { campaignsRepository } =
        await import("~/api/repositories/campaigns_repository");
      vi.mocked(campaignsRepository.get).mockRejectedValueOnce(
        new Error("Campaign not found"),
      );

      const store = useCampaignsStore();

      await expect(store.fetchCampaign("invalid-id")).rejects.toThrow(
        "Campaign not found",
      );
  ...
  describe("updateCampaign - edge cases", () => {
    ...
    test("should not update selectedCampaign if different campaign", async () => {
      ...
      vi.mocked(campaignsRepository.get).mockResolvedValueOnce(
        campaign1 as unknown as Awaited<
          ReturnType<typeof campaignsRepository.get>
        >,
      );
    ...
    test("should handle update error", async () => {
      const { campaignsRepository } =
        await import("~/api/repositories/campaigns_repository");
      vi.mocked(campaignsRepository.update).mockRejectedValueOnce(
      ...
      await expect(
        store.updateCampaign("1", { name: "New Name" }),
  ...
  describe("createCampaign - edge cases", () => {
    test("should handle create error", async () => {
      const { campaignsRepository } =
        await import("~/api/repositories/campaigns_repository");
      vi.mocked(campaignsRepository.create).mockRejectedValueOnce(
      ...
      await expect(
        store.createCampaign({ name: "", description: "" }),
  ...
  describe("deleteCampaign - edge cases", () => {
    test("should handle delete error", async () => {
      const { campaignsRepository } =
        await import("~/api/repositories/campaigns_repository");
      vi.mocked(campaignsRepository.delete).mockRejectedValueOnce(
        new Error("Delete forbidden"),
      );

      const store = useCampaignsStore();

      await expect(store.deleteCampaign("1")).rejects.toThrow(
        "Delete forbidden",
      );
